...
class _x_teamMemberState extends State<x_teamMember> {
  ...
  void _onDone() {
    final List<Map<String, String>> newMembersData = [];
    bool allFormsValid = true;

    // Iterate through all member forms to validate and collect data.
    for (var formData in _memberForms) {
      if (formData.formKey.currentState!.validate() &&
        ...
        allFormsValid = false;
        break; // Stop and show error if any form is incomplete
      }
    }

    if (allFormsValid) {
      print('New Members Data: $newMembersData');
      // Navigate back to x_teamRescue.dart with the list of new members.
      Navigator.pop(context, newMembersData);
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please fill out all fields for all members.'),
                    ...
                    child: ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(
                        ...
                        shape: RoundedRectangleBorder(
                          // The DONE button now has more rounded corners.
